...
*子网掩码*同样为32bit，通过将对应位置 bit 置1表示子网 IP 地址中的该位表示子网部分，将子网掩码和 IP 地址按位 AND 计算，就可得到网络号
...
*分类编址*(classful addressing)架构中，IP 地址分为 ABCDE 五类
...
ISP从*因特网名字和编号分配机构ICANN*获取IP地址块

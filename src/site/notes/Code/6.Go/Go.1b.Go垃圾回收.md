---
{"dg-publish":true,"permalink":"/Code/6.Go/Go.1b.Go垃圾回收/","title":"垃圾回收","noteIcon":""}
---
...
Go 的编译器会通过**逃逸分析**将大部分新生对象存储在栈上(栈直接被回收)，只有那些需要长期存在的对象才会被分配到需要进行垃圾回收的堆中
...
由于不对栈空间进行插入写屏障，因此在完成扫描后需要启用STW对栈重新进行三色标记扫描
...
**问题**：回收精度低，一次GC中一个对象即使被删除了最后一个引用也依旧可以存活，在下一轮GC中被清理掉

### V1.8 混合写屏障机制

操作：
- GC开始时扫描栈对象，将可达对象全部标记为黑色
- GC期间栈上创建的新对象均为黑色
- 被删除的对象标记为灰色
- 被添加的对象标记为灰色

...
![[Database.0a.Redis基础数据结构\|Database.0a.Redis基础数据结构]]
...
不淘汰：
- `noeviction` (default): 不淘汰，内存超过限制之后写入操作会触发 OOM，但没有新写入时读操作和删除操作正常进行
...
- *AOF*(Append Only File)，记录执行的**每条写操作命令**，重启之后通过重放命令来恢复数据，本质是记录操作日志，后续通过日志重放恢复数据
...
V2.6 后 Redis 通过内嵌支持 Lua 环境，通过*EVAL*命令执行脚本

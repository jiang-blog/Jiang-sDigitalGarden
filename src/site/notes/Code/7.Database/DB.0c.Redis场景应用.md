...
通过将数据备份到其他服务器上可避免单点故障问题

Redis 提供主从复制模式以解决多服务器间的数据一致性问题

主服务器可以进行读写操作，当发生写操作时自动将写操作同步给从服务器
从服务器一般是只读，同时接受主服务器同步过来的写操作命令并执行
...
1. 建立链接、协商同步
    ...
    2. 主服务器返回 `FULLRESYNC` 响应命令，表示**采用全量复制方式**同步数据
2. 主服务器同步数据给从服务器
    1. 主服务器执行 BGSAVE 命令生成 RDB 文件并发送给从服务器
    2. 从服务器收到后清空当前数据，载入 RDB 文件
3. 主服务器发送新写操作命令给从服务器
    1. 从服务器载入 RDB 后回复确认消息给主服务器
    2. 主服务器将 `replication buffer` 缓冲区内记录的写操作命令发送给从服务器
        - 主服务器生成 RDB 文件期间
        - 主服务器发送 RDB 给从服务器期间
        - 从服务器加载 RDB 文件期间
    3. 从服务器依次执行命令，完成第一次同步
...
主从服务器在完成第一次同步后，双方之间就会维护一个 *TCP 长连接*，后续主服务器可以通过这个连接继续将写操作命令传播给从服务器，然后从服务器执行该命令，使得与主服务器的数据库状态相同
...
主从服务器断开连接后重新连接，V2.8以后主从服务器会采用*增量复制*的方式继续同步，也就是只会把网络断开期间主服务器接收到的写操作命令同步给从服务器

增量复制步骤：
1. 从服务器在恢复网络后发送 `psync` 命令给主服务器
2. 主服务器用 `CONTINUE` 响应命令，表示**采用增量复制的方式**同步数据
3. 主服务器发送网络断开期间主服务器接收的写操作命令，从服务器执行命令

服务器会将写命令写入到 repl_backlog_buffer **环形缓冲区**里，并用 `repl offset` 标记缓冲区中当前写入写命令的位置
Redis 主节点每次收到写命令之后，先写到内部的缓冲区，然后**异步发送给从节点**

增量复制过程中从服务器的 `psync` 命令中的 `offset` 为自己的复制偏移量 `slave_repl_offset`，主服务器根据自己的 master_repl_offset 和 slave_repl_offset 之间的差距，然后来决定对从服务器执行哪种同步操作：
- 从服务器要读取的数据还在 repl_backlog_buffer 缓冲区里，主服务器将采用增量同步
- 从服务器要读取的数据不在 repl_backlog_buffer 缓冲区里，主服务器将采用全量同步
...
Redis 判断节点是否正常工作主要通过互相的 ping-pong 检测，如果有一半以上的节点去 ping 一个节点的时候没有 pong 回应，集群就会认为这个节点挂掉了，会断开与这个节点的连接
...
问题：
- 从服务器数量多时主服务器会忙于使用 fork()创建子进程进而阻塞主线程
- 传输 RDB 文件占用主服务器的网络带宽，影响主服务器响应命令
解决方法：
...
哨兵的作用是实现**主从节点故障转移**，哨兵监测主节点是否存活，如果发现主节点挂了，它就会选举一个从节点切换为主节点，并且把新主节点的相关信息通知给从节点和客户端
